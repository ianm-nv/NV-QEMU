...
int kvm_arch_init(MachineState *ms, KVMState *s)
{
    ...
    for (i = 0; i < ARRAY_SIZE(filter_ranges); i++) {
        ...
        if (kvm_vm_ioctl(s, KVM_SET_DEVICE_ATTR, &attr))
            warn_report("KVM_SET_DEVICE_ATTR failed");
    }
...
/* 0 -> return to guest, 1 -> grace exit, -1 -> panic/die */
...
static int handle_da_object_read(ARMCPU *cpu)
{
    ...
    hwaddr len = 4096;
    void *hva = cpu_physical_memory_map(guest_ipa, &len, true);
    int ret;
...
static int handle_da_get_measurements(ARMCPU *cpu)
{
    ...
    param = env->xregs[2];
    param_hva = (struct rhi_vdev_measurement_params *)cpu_physical_memory_map(param, &len, false);
...
static int handle_arm64_tio_exit(ARMCPU *cpu, struct kvm_run *kvm_run)
{
    bool ret;
    if (kvm_run->cca_exit.nr == RMI_EXIT_VDEV_MAP) {
        ...
        ret = iommufd_tsm_dev_memmap_exit(kvm_vcpu_fd(CPU(cpu)),
                          vdev_id, gpa_base, gpa_top, pa_base);
        if (!ret)
            kvm_vcpu_ioctl(CPU(cpu), KVM_ARM_VCPU_RMM_REC_ENTER_FLAGS,
                  REC_ENTER_FLAG_DEV_MEM_RESPONSE);

    } else if (kvm_run->cca_exit.nr == RMI_EXIT_VDEV_REQUEST) {
        iommufd_tsm_vdev_req_exit(kvm_vcpu_fd(CPU(cpu)), kvm_run->cca_exit.vdev_id);
    }
    return 0;
}
...
int kvm_arch_handle_exit(CPUState *cs, struct kvm_run *run)
{
    ...
    case KVM_EXIT_ARM64_TIO:
        ret = handle_arm64_tio_exit(cpu, run);
        break;
